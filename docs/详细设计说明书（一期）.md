 ...
 **1. 请求URL**
 ...
 –	请求URL：http://192.168.1.107/user/accountInformation
 –	请求方式：GET
 –	参数如下：
 
 |参数名|必选|类型|说明|
 |-----  |----- |----- |-----   |
 |accSn |是  |String | 用户唯一ID|
 ...
 **2. 响应实例**
   ...
   **1. 请求URL**
   ...
   –	请求URL：http://192.168.1.107/content/algorithmRecord
   –	请求方式：POST
   –	参数如下：
   
   |参数名|必选|类型|说明|
   |-----  |----- |----- |-----   |
   |callUsrSn |是  |String | 调用用户串号我调用别人算法|
   |page |是  |String | 页数|
   ...
   |callPayAmount |String | 费用|
    ...
    ### 3.5	收益情况
      **1. 请求URL**
      ...
      –	请求URL：http://192.168.1.107/content/algorithmEarnRecord
      –	请求方式：POST
      –	参数如下：
